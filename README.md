...
To add a video to a specific item, open **Edit → Product video**, choose **Add video** (or **Replace video**) and select an MP4/WebM file up to 50 MB, then **Save changes**. You can also paste a YouTube or public video link. The editor previews selected files and shows upload progress; failed uploads can be retried without uploading completed media again. The same controls are available when adding a product. Videos appear after photos on the product page and remain excluded from category PDFs.
...
In `/admin`, use **Download a category catalogue** to choose a category and download a PDF containing each card's print name (or design number when no print name is set), design number, and up to four main/gallery photos. Videos are excluded before choosing the first four unique photos. **Only cards with photos** is on by default: cards without either a main photo or gallery photo are skipped, and the download confirmation reports how many were skipped. Turn it off to include those cards. The export covers the selected category for the selected company, regardless of library search or pagination. Website collections include active, website-visible cards; item categories also include hidden and disabled items. Christian and Muslim collections follow the site's existing shared `Wedding Card` mapping and both use **Wedding Cards** as their PDF title.

PDFs are generated in the browser on demand, with one page per card and at most four photos. Progress updates are throttled to 400 ms, with browser yields every ten pages. An eight-request pool prefetches six cards ahead, downloads and embeds each unique photo URL once, and stops queued work on cancellation. Repeated text layouts and Unicode text images are cached within each export, and saving uses `useObjectStreams: false`. Gallery reads use parallel chunks of 200 items. Missing or inaccessible photos are marked in the PDF and reported after download; remote image hosts must permit cross-origin image access (as Supabase public storage does).

Public Supabase photos use `render/image/public/` with `width=1000&quality=72`, falling back to the original URL if transformation fails. These requests count toward the project's [Supabase image transformation usage](https://supabase.com/docs/guides/storage/serving/image-transformations); existing uploads and `thumb_url` values are unchanged. Modern browsers fetch and decode images with `createImageBitmap`, then encode JPEG bytes asynchronously with `OffscreenCanvas.convertToBlob`. Browsers without those APIs use `Image` and `canvas.toBlob` as a compatibility path, also passing raw bytes to the PDF library.
