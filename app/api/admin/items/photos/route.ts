...
import { appendItemPhoto, replaceItemPhoto } from "@/lib/admin/item-photo";
...
export async function POST(request: NextRequest) {
  if (request.headers.get("origin") !== new URL(request.url).origin)
    return NextResponse.json(
      { error: "Open the dashboard on this site to add photos." },
      { status: 403, headers },
    );
  if (
    Number(request.headers.get("content-length")) >
    MAX_PRODUCT_PHOTO_BYTES + 128 * 1024
  )
    return NextResponse.json(
      { error: "Choose photos up to 3 MB each." },
      { status: 413, headers },
    );
  try {
    let form: FormData;
    try {
      ...
      throw new ItemCreateError("Choose a photo to upload.");
    }
    const companyId = String(form.get("companyId") ?? "");
    const itemId = String(form.get("itemId") ?? "");
    const uploadId = String(form.get("uploadId") ?? "");
    const previousUrl = form.get("replaceUrl");
    ...
    if (
      ...
      (typeof previousUrl !== "string" ||
        !previousUrl.trim() ||
        previousUrl.length > 2000)
    )
      throw new ItemCreateError("Choose an existing photo to replace.");
    if (
      !(photo instanceof File) ||
      !photo.size ||
      photo.size > MAX_PRODUCT_PHOTO_BYTES
    )
    ...
    try {
      const saved =
        typeof previousUrl === "string"
          ...
          : await appendItemPhoto(photo, companyId, itemId, uploadId);
