...
/**
 * Cart page. Fully client-rendered since it depends on cart state.
 * Shows the item list with quantity controls, an order summary, and
 * a checkout button (currently a placeholder — wire to ERPNext later).
 */
export default function CartPage() {
    const { items, removeItem, setQuantity, clearCart, totalItems, totalPrice } =
        useCart();

    // Track images that failed to load, so we can show the emoji fallback.
    ...
    function handleCheckout() {
        // TODO: connect to ERPNext — call createErpSalesOrder() from lib/erpnext.ts
        // with the cart items, then redirect to an order-confirmation page.
        alert(
            `Order placed for ${totalItems} item(s) — ₹${formatPrice(totalPrice)}.\n` +
            "Checkout is a placeholder; connect ERPNext to process real orders.",
        );
        clearCart();
    }
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr_320px]">
                ...
                <div className="h-fit rounded-xl border border-gold/25 bg-white p-5">
                    ...
                    <button
                        ...
                        className="mt-5 w-full rounded-lg bg-maroon py-3 text-sm font-semibold text-gold-light transition hover:bg-maroon-dark"
                    >
                        Proceed to Checkout
                    </button>
