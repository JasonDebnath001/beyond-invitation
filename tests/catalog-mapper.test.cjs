...
const image = (name) => `https://ldjcivtrbmxmkemjqqdi.supabase.co/storage/v1/object/public/item_images/${name}.jpg`;
const row = (changes = {}) => ({
  id: "item-id", item_code: "IT0419", design_no: "535093", slug: "535093",
  name: "Invitation", description: "A wedding invitation", price: 90, mrp: 100,
  gst_pct: 18, image_url: image("primary"), thumb_url: image("thumb"),
  images: [image("primary"), image("second")], videos: [], subject: "Wedding Card",
  tags: ["New"], badge: null, stock_status: "Available", min_order_qty: 50,
  order_multiple: 25, group_name: "Supplier / Year", width_mm: 125, length_mm: 200,
  height_mm: 3, weight_g: 40, updated_at: "2026-09-14T00:00:00Z", ...changes,
});

function loadCatalog(rows = [row()]) {
  const state = { requests: [], margin: 0, cacheKeys: [] };
  const applyPrice = (product) => product && ({ ...product, price: product.price * (1 + state.margin / 100) });
  const catalog = loadModule("lib/catalog.ts", {
    ...
    "@/lib/reseller": {
      applyResellerPricingToProducts: async (products) => products.map(applyPrice),
        ...
        from(table) {
          ...
          return {
            ...
            async returns() {
              ...
              return { data: rows.slice(request.range[0], request.range[1] + 1), error: null };
            },
          };
        },
      }),
...
test("null price maps to zero with hasPrice false and no invented MRP", () => {
  ...
  const product = catalog.mapCatalogRowToProduct(row({ price: null, mrp: null }));
...
test("images are deduplicated with the primary first, without using the thumbnail as a gallery image", () => {
  const { catalog } = loadCatalog();
  const product = catalog.mapCatalogRowToProduct(row({ images: [image("second"), null, "", image("primary"), image("second")] }));
  assert.deepEqual(Array.from(product.images), [image("primary"), image("second")]);
  assert.deepEqual(Array.from(catalog.mapCatalogRowToProduct(row({ image_url: null, images: null })).images), []);
});
...
test("unknown and absent Subjects fall back to wedding, never the supplier group", () => {
  ...
  for (const subject of [null, "", "Hindi", "Common", "Invitation"]) {
    const product = catalog.mapCatalogRowToProduct(row({ subject, group_name: "Luxury supplier" }));
...
test("category keyword mapping uses Subject", () => {
  ...
  for (const [subject, category] of [
    ["Luxury", "luxe"], ["Luxe", "luxe"], ["House Warming", "housewarming"],
    ["Thread Ceremony", "thread-ceremony"], ["Naming", "naming-ceremony"],
    ["Birthday", "birthday"], ["Baby Shower", "baby-shower"],
  ]) assert.equal(catalog.mapCatalogRowToProduct(row({ subject })).category, category);
});

test("slug passes through unchanged; item identities use the design number", () => {
  const { catalog } = loadCatalog();
  const product = catalog.mapCatalogRowToProduct(row({ slug: "existing-535093" }));
...
test("numeric prices, millimetres, grams, tags and order metadata are preserved", () => {
  const { catalog } = loadCatalog();
  const product = catalog.mapCatalogRowToProduct(row({ price: "90.50", gst_pct: "18" }));
  assert.equal(product.price, 90.5);
  assert.equal(product.hasPrice, true);
  assert.equal(product.badge, "New");
  assert.equal(catalog.mapCatalogRowToProduct(row({ badge: "Offer" })).badge, "Offer");
  assert.deepEqual(plain(product.dimensions), { height: 3, width: 125, depth: 200, weight: 40 });
...
test("all list/detail/subject reads share a cached base list without caching reseller margins", async () => {
  ...
  const [base, list, detail, subjects] = await Promise.all([
    catalog.fetchErpProductsBase(), catalog.fetchErpProducts(),
    catalog.fetchErpProductBySlug("535093"), catalog.fetchErpProductsBySubject("wedding card"),
  ]);
...
test("subject filters are exact and case-insensitive; language/unassigned subjects stay out of collections", async () => {
  const { catalog } = loadCatalog([
    ...
    row({ slug: "hindi", subject: "Hindi" }), row({ slug: "none", subject: null }),
    row({ slug: "partial", subject: "Hindu Wedding Card Extra" }),
  ]);
  const result = await catalog.fetchErpProductsBySubject([" hindu wedding CARD ", "wedding card"]);
  assert.deepEqual(Array.from(result, p => p.slug), ["hindu", "shared"]);
  assert.deepEqual(Array.from(await catalog.fetchErpProductsBySubject([])), []);
  assert.deepEqual(Array.from(await catalog.fetchErpProductsByCategory("wedding"), p => p.slug), ["hindu", "shared"]);
...
test("range pagination returns every row with stable updated_at ordering", async () => {
  const rows = Array.from({ length: 1001 }, (_, i) => row({ id: String(i), slug: String(i) }));
  const { catalog, state } = loadCatalog(rows);
  assert.equal((await catalog.buildErpProductList()).length, 1001);
  assert.deepEqual(plain(state.requests.map(request => request.range)), [[0, 999], [1000, 1999]]);
  assert.deepEqual(plain(state.requests[0].orders), [["updated_at", { ascending: false }], ["id", { ascending: true }]]);
});
...
test("checkout rejects an unpriced line even when mixed with a paid item", async () => {
  ...
  const unpriced = catalog.mapCatalogRowToProduct(row({ slug: "unpriced", design_no: "unpriced", price: null }));
  const checkout = loadCheckout([paid, unpriced]);
  await assert.rejects(checkout.resolveCartProducts([
    { slug: paid.slug, quantity: 50 }, { slug: unpriced.slug, quantity: 50 },
...
test("checkout uses base catalogue prices and the existing reseller margin once", async () => {
  ...
  const result = await checkout.resolveCartProducts([{ itemCode: "535093", quantity: 50 }], { active: true, marginPercent: 10 });
...
test("server client initializes lazily and applies the 60-second fetch policy", async () => {
  ...
  }, { process: { env }, fetch: async (_, init) => { fetchOptions = init; } });
  assert.equal(created, 0);
  assert.throws(() => server.getSupabaseServerClient(), /NEXT_PUBLIC_SUPABASE_URL/);
  ...
  await options.global.fetch("https://example.supabase.co", { headers: { apikey: "test-anon-key" } });
